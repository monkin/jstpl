...
function compiler(loader) {
	...
	compilerObject = {
		...
		appendPlugin: function(name, fileName) {
			if(!plugins[name]) {
				plugins[name] = true
			...
			if(!dependencies[fileName]) {
				...
				loadFile(fileName, function(content) {
					...
					for(i in parsed.includes) {
						compilerObject.includeFile(parsed.includes[i])
					}
					for(i in parsed.dependencies) {
						compilerObject.parseFile(parsed.dependencies[i])
					}
				})
			}
		},
			...
			cnt.wait(function(err) {
				if(err)
					...
					callback(null, "(function (fileName, ctx) { "
						...
						+ "function varCall(v, ctx, args, out) { if(v && typeof v=='function') v(ctx, args, out); } "
						+ "var $ctx = context(ctx); "
